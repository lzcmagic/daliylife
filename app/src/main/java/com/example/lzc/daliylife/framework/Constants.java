package com.example.lzc.daliylife.framework;

/**
 * Created by lzc on 2016/12/5.
 */

public class Constants {
    public static String FragmentTagNews="news";
    public static String FragmentTagWeChart="wechart";
    public static String FragmentTagDaliy="daliy";
    public static String NewsFragmentTagHot="news_hot";
    public static String NewsFragmentTagSport="news_sport";
    public static String NewsFragmentTagHouse="news_house";
}
